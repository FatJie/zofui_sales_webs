            ...
            <Col span="24">
                <Table ref="selection" :columns="columns" :data="list" @on-select="selectTable" @on-select-cancel="selectTable" @on-select-all="selectTable" :loading="tableloading">
                    ...
                    <template slot-scope="{ row }" slot="fee">
                        <p>纯利润:{{row.chunlirun}}</p>
                        <p>让利金额:{{row.ranglifee}}</p>
                        <p>赚取金额:{{row.earnfee}}</p>
                    </template>
                    ...
                    <template slot-scope="{ row }" slot="baozhengjin">
                        <p v-if="row.authentication==0">保证金：未缴纳</p>
                        <p v-if="row.authentication>0">保证金：已缴纳{{row.authentication}}元</p>
                    </template>
                    ...
                    <template slot-scope="{ row, index }" slot="action">
                        ...
                        <div>
                            ...
                            <a v-if="row.ischain == 0" @click="dealthis(row.id,'hidechain')"  >隐藏连锁</a>
                            <a class="font-error" v-if="row.ischain == 1" @click="dealthis(row.id,'showchain')"  >显示连锁</a>
                        ...
                        <Dropdown placement="left" trigger="click">
                            ...
                            <DropdownMenu slot="list">
                                <div style="padding:10px">
                                    <Input v-model="money" placeholder="输入数值" />
                                    <p class="frm_tip">0：退保证金；填写大于0为缴纳保证金金额</p>
                                </div>
                                <div style="text-align: right;margin:10px;">
                                    <Button type="primary" size="small" @click="editmoney(row.id,'changebaozhengjin')">确定</Button>
                                </div>
                            </DropdownMenu>
                        </Dropdown>
	  	...
	  	data : function(){
	  		return {
                ...
                columns : [
                    ...
                    {title:'数据',key:'money',slot: 'money',},
                    {title:'数据分析',key:'fee',slot: 'fee',},
                    {title:'状态',key:'status',slot: 'status',},
                    {title:'店铺认证-保证金',key:'baozhengjin',slot: 'baozhengjin',},
