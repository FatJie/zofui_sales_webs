<template>
	<div>
        ...
        <Tabs value="adminsignjoinlog" @on-click="changetab">
            ...
            <Col span="24">
                <Table ref="selection" :columns="columns" :data="list" @on-select="selectTable" @on-select-cancel="selectTable" @on-select-all="selectTable">
                    ...
                    <template slot-scope="{ row }" slot="status">
                        ...
                        <template v-else>
                            <p v-if="row.istaked == 1">已发奖</p>
                            <p v-else>未发奖{{row.err}}</p>
                        </template>
                    ...
                    <template slot-scope="{ row, index }" slot="action">
                        <div>
                            <p>
                                <a @click="link" :data-url="'adminsignsignlog?actid='+row.id"  >签到记录</a>
                            </p>
                            <Poptip
                                confirm
                                title="确定删除吗？"
                                @on-ok="editmoney(row.id,'deletejoin')"
                                placement="left">
                                <a>删除</a>
                            </Poptip>
                            <Poptip
                                v-if="row.status == 1 && (row.istaked == 0 || row.istaked == '0')"
                                confirm
                                title="确定补发奖品吗？"
                                @on-ok="dealthis(row.id,'sendsignprize')"
                                placement="left">
                                <a>补发奖品</a>
                            </Poptip>
                        </div>
        ...
        props : ['settings'],
	  	data : function(){
	  		return {
                ...
                pass1 : '',
                pass2 : '',
                status : [{label:'全部',value:0},{label:'未完成',value:1},{label:'已完成',value:2},],
	  		}
	  	},
        created:function(){
            this.initData();
	  	...
	  	methods: {
            ...
            editmoney(id,type){
                ...
                self.$util.http('signin','signin',postdata,function(res){
                    if( res.status == 200 ){
                        self.$Message.success(res.res);
                        self.onLoad();
                        self.money = 0;
