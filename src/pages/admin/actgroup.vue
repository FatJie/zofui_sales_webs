            ...
            <Col span="24">
                <Table ref="selection" :columns="columns" :data="list" @on-select="selectTable" @on-select-cancel="selectTable" @on-select-all="selectTable">
                    ...
                    <template slot-scope="{ row }" slot="status">
                        ...
                        <p v-if="row.dismem > 0 && row.end > nowtime">进行中</p>
                        <p v-else-if="row.dismem <= 0">已完成</p>
                        <p v-else-if="row.dismem > 0 && row.end < nowtime">已失败</p>
                    ...
                    <template slot-scope="{ row, index }" slot="action">
                        ...
                        <div>
                            ...
                            <a @click="link" :data-url="'adminorder?groupid='+row.id" >查看订单</a>
                        </div>
                    </template>
                </Table>
        ...
        created:function(){
	  	...
	  	methods: {
            ...
            initData(){
                ...
                if( params.uid > 0 ){
                    self.page.pdata.userid = params.uid;
                }
                if( params.id > 0 ){
