<template>
	<div>
        ...
        <Tabs value="adminsign" @on-click="changetab">
            ...
            <Col span="24">
                <Table ref="selection" :columns="columns" :data="list" @on-select="selectTable" @on-select-cancel="selectTable" @on-select-all="selectTable">
                    ...
                    <template slot-scope="{ row, index }" slot="action">
                        ...
                        <a @click="link" :data-url="'adminsignadd?id='+row.id" >编辑</a>
                        <Poptip
                            confirm
                            title="确定删除吗？"
                            @on-ok="dealthis(row.id,'deletesignin')"
                            placement="left">
                            <a>删除</a>
                        </Poptip>
                        <Poptip
                            confirm
                            title="确定复制活动吗？"
                            @on-ok="editmoney(row.id,'copyact')"
                            placement="left">
                            <a>复制活动</a>
                        </Poptip>
                        <div>
                            <a @click="link" :data-url="'adminsignjoinlog?id='+row.id" >参与记录</a>
                        </div>
                        <div>
