    ...
    <div>
        <Row style="margin-bottom:10px;">
            <Col span="24">
                ...
                <Button type="primary" v-if="op == 'card'" @click="down">导出订单</Button>
            </Col>
        </Row>
        <Row>
            <Col span="24">
                <Table ref="selection" :columns="columns" :data="list" @on-select="selectTable" @on-select-cancel="selectTable" @on-select-all="selectTable">
                    ...
                    <template slot-scope="{ row }" slot="data">
                        ...
                        <p v-if="row.cardfee > 0">优惠券优惠:{{row.cardfee*1}}</p> -->
                        
                    </template>
                    <template slot-scope="{ row }" slot="other">
                        <p>{{row.jiesuan}}</p>
                        <!-- <p v-if="row.paytype == 0">微信支付</p>
                        <p v-if="row.paytype == 1">余额支付</p>
	    ...
	    data : function(){
            return {
                ...
                columns : [
                    ...
                    {title:'金额',key:"fee",},
                    {title:'让利金额',key:"data",slot: 'data',},
                    {title:'结算金额',key:'other',slot: 'other',},
                    {title:'状态',key:'status',slot: 'status',},
                    {title:'支付时间',key:'33',slot: 'action',},
                ],
